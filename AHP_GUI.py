...
def ahp(pheromone_list):
    ...
    names = ['Wysoka ocena atrakcji', 'Dobre dopasowanie kategorii', 'Niska cena', 'Wysoka popularnosc']
    maxlen = 0
    for i in names:
        if len(i)>maxlen:
            maxlen=len(i)
    layout = [[sg.Text('Preferencje kryteriow')]]
    text = []
    text1 = []
    text2 = []
    ij = []
    ...
    layout.append([sg.Button('Gotowe')])
    window = sg.Window('Preferencje kryteriow', layout)
    while True:
