...
with open('places.csv') as file:
    ...
    for i in csv_reader:
        x = main.Place(str(i[0]), float(i[4]), float(i[5]), float(i[6]), float(i[3]), i[7],i[8], categories=main.get_categories(i[2]), price=float(i[1]), popularity=int(i[9]))
...
layout = [
    ...
    [sg.Text('Miejsce rozpoczecia podrozy:', size=(22, 1)), sg.InputText('Hôtel Malte - Astotel', key='address')],
...
while True:
    ...
    if event == "Gotowe":
        ...
        for i in places:
            ...
            c+=1
            routes.add_route(place, i, float(get_route(place.lat,place.lan,i.lat,i.lan)['routes'][0]['distance'])/5/1000,10)
